...
- 2026-07-20 profile achievements pass: the public profile page (`frontend/src/pages/ProfilePage.tsx`) only ever rendered the 3 showcased achievement slots, with no way to see the rest of the catalog from a profile. Extracted the hub's existing achievements-modal rendering (filter toolbar, cards, progress bars) into a shared `frontend/src/features/achievements/AchievementGrid.tsx`, driven entirely by the same `GET /achievements` catalog (`api.getAchievements()`) both surfaces already fetched — no page-local copy of achievement metadata anywhere. `ProfilePage` now renders the full catalog (locked and unlocked) below the showcase, and `HomePage`'s Achievements modal was refactored onto the same component instead of its own inline copy, so the two can no longer drift apart. New regression test in `ProfilePage.test.tsx` guards the full-catalog rendering; full frontend suite (414 tests) and both builds green; verified live against the running stack (Firefox headless) on both the profile page and the hub modal.
