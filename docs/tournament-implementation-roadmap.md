...
> - **F2 Motores — COMPLETA.** Los seis motores construidos y revisados contra sus SPECs: Economy (011), Rule Engine (009, 5 modificadores), Leaderboard (018), Action Engine + Registry/Factory (008), Inventory + Item Framework (014/007), Reward Resolver (013). Integración en el Runtime (`runtime/tournament-engines.ts`): se construyen por-torneo, se cablean las costuras diferidas (adaptador Rule→Economy `RewardRuleApplier`; bundle `ActionServices`; un único runner sobre el Action Engine que satisface `ItemEffectRunner` + `RewardActionRunner`; `GrantItemAction` que estrecha `InventoryPort`) y se serializan en el snapshot del Runtime. Checkpoint cumplido: Reward compuesta (puntos+item) acredita wallet y llena inventario end-to-end.
> - **F3 Gameplay Base — COMPLETA.** Board System (002) con modelo de Tile (datos), tablero anillo v1 de sucesor único (`board/`, `movePlayer`/`teleportPlayer`, límite anti-bucle de relocalización, jugadores comparten casilla, resolución de Tile delegada al Action Engine vía `TileActionRunner`); Dice System (010, `dice/`, dado=lista de números, tirada sembrada reproducible + `rollCount`, costuras `DiceValueModifier`/`ActiveDieResolver`, catálogo normal/chiquito/grande/op); Tile Actions (006) `nothing`/`teleport`/`movePlayer` (estrechan `BoardPort`); Turn System (005, `turn/`, un turno activo, PlayerTurnStarted→DiceRollRequested→roll→move→resolve→PlayerTurnFinished, timeout con auto-tirada vía reloj inyectable, auto-resolución por desconexión). Todo cableado en `tournament-engines.ts` (Board+Dice+TurnSystem por-torneo, `services.board`, Dice→`queryDiceModifier`, seed hilado desde el Runtime) y serializado en el snapshot; PRNG compartido extraído a `infra/seeded-rng.ts`. **Checkpoint cumplido:** una ronda completa de turnos de tablero simulada end-to-end (4 jugadores tiran, mueven y resuelven casilla) en `tournament-engines.spec.ts`. Suite backend 1136 verde; `tsc` limpio; drift de contratos verde.
> - **Siguiente: HITO Vertical Slice** (networking snapshot-first + UI mínima) — requiere netcode de arena (gate D9) y decisiones de intents/salas.
