...
**Date:** 2026-07-07
**Scope:** The full gambling subsystem as mapped by `graphify-out/graph.json` — backend `backend/src/modules/casino/` (wheel, flip, monte, slots, dice, plinko + shared engine/fairness + wager audit table), frontend `frontend/src/components/casino/` (six modals + pure-logic helpers), the API layer in `frontend/src/features/hub/api.ts`, wiring in `frontend/src/pages/HomePage.tsx`, and the adjacent modules that share the `users.coins` wallet (cards, customization, game-results).
...
- `runBoardAnimation`'s cancel function sets `cancelled = true` and never invokes `onComplete` — `frontend/src/components/casino/board-canvas.ts:170-173`.
- Each modal calls `onCoinsChange` only inside its `finish()` callback, which is `runBoardAnimation`'s `onComplete`:
  - `FortuneWheelModal.tsx:200-221` (finish) + unmount cleanup cancels at `:175-178`
  - `KoiDiceModal.tsx:180-224` (effect cleanup returns `cancel` at `:223`)
  - `ShellFlipModal.tsx:174-231`
  - `ThreeShellMonteModal.tsx:270-316`
  - `ShrineSlotsModal.tsx:243-314`
  - `ShellDropModal.tsx:300-336`
...
`ShellDropModal.tsx`: after a drop lands, the tier buttons re-enable, but the rendered state still derives from the *current* tier:
...
Monte already solved this: `changeShells` clears `result`/`verify`/`finalPositions` (`ThreeShellMonteModal.tsx:319-326`).
...
- **No double-spend via HTTP retries.** `apiFetch` retries non-GET only when `idempotent: true`, which no casino call sets (`frontend/src/features/hub/api.ts:102-117`); the CSRF-failure replay only fires when the original request was rejected *by CSRF* and therefore never executed.
...
- **`verifyDice` is immune to post-roll slider changes.** It ignores `direction`/`target` (underscored params, `fairness.ts:201-209`) and recomputes only the rolled value. Similarly `verifyMonte` receives `playedShells` captured per round (`ThreeShellMonteModal.tsx:537`), not live state.
- **Client fairness maths mirrors the server.** `dice.ts`/`flip.ts`/`monte.ts`/`plinko.ts`/`slots.ts`/`wheel.ts` were diffed against the backend constants — thresholds, clamps and outcome-id formats match, including the single-roll vs multi-roll HMAC message scheme (`fairness.ts:150-171` vs `casino.fair.ts`).
