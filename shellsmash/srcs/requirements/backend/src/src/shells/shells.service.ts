...
export class ShellsService {
  ...
  async seedInventory(user: User): Promise<void> {
    try {
      ...
      console.error('[ShellsService] Failed to seed inventory for user', user.id, err);
    }
  }
  ...
  async getInventory(userId: number): Promise<Record<string, number>> {
    try {
      ...
      throw new InternalServerErrorException('Failed to fetch shell inventory');
    }
  }
   ...
   */
  async validateSelection(userId: number, shellTypes: string[]): Promise<string[]> {
    ...
    for (const type of specials) {
      if ((inventory[type] ?? 0) < 1) {
        throw new BadRequestException(`Shell not in inventory: ${type}`);
      }
    }
