...
import { BambooBashSnapshot, GameInputPayload, MatchRoom, RoomPlayer, SnapshotPlayer } from '../matchmaking.types';
...
export class BambooBashEngine implements GameEngine {
  ...
  createInitialState(context: GameEngineCreateContext, roomPlayers: RoomPlayer[]): BambooBashSnapshot {
    return {
  ...
  handleInput(room: MatchRoom, userId: number, input: GameInputPayload): MatchRoom | null {
    ...
    if (input.action === 'bamboo:hit') return this.applyBambooHit(room, userId, input.payload ?? {});
    if (input.action === 'bamboo:sync') return this.applyBambooSync(room, userId);
  ...
  private applyBambooHit(room: MatchRoom, userId: number, payload: Record<string, unknown> = {}): MatchRoom | null {
    const state = room.state as BambooBashSnapshot;
    this.updateSharedBamboos(state);
    const player = room.players.find((candidate) => candidate.user.id === userId);
    if (!player || room.status !== 'active' || state.phase !== 'active') return null;
    ...
    if (roundNumber !== state.roundNumber || !Number.isFinite(bambooId)) return null;

    const index = state.bamboos.findIndex((bamboo) => bamboo.id === bambooId);
  ...
  abandon(room: MatchRoom, abandonedPlayer: RoomPlayer): number | null {
    ...
    const maxScore = Math.max(...connectedScores.map((entry) => entry.score));
    const winners = connectedScores.filter((entry) => entry.score === maxScore);
  ...
  private resetSharedBamboos(state: BambooBashSnapshot, playerCount: number): void {
  ...
  private updateSharedBamboos(state: BambooBashSnapshot): void {
    ...
    for (const bamboo of state.bamboos) {
      bamboo.ageMs += delta;
      bamboo.stage = Math.min(MAX_STAGE, 1 + Math.floor(bamboo.ageMs / GROW_INTERVAL_MS));
    }
  ...
  private spawnBamboo(state: BambooBashSnapshot): void {
    ...
    if (!spot) return;
    state.bamboos.push({ id: state.nextBambooId++, nx: spot.nx, ny: spot.ny, stage: 1, ageMs: 0 });
  }

  private randomSpot(existing: BambooBashSnapshot['bamboos']): { nx: number; ny: number } | null {
    ...
    for (let attempt = 0; attempt < 24; attempt++) {
      ...
      if (existing.some((candidate) => Math.hypot(candidate.nx - nx, candidate.ny - ny) < minSep)) continue;
      return { nx, ny };
    }
    return null;
  }
