 ...
 */
export function arenaEdgeFraction(px: number, py: number, a: ArenaPixels): number {
  const dx = (px - a.cx) / a.rx;
  const dy = (py - a.cy) / a.ry;
 ...
 */
export function drawSumoRing(g: Phaser.GameObjects.Graphics, a: ArenaPixels): void {
