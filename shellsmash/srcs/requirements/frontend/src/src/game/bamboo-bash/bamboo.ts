...
export function hitsBamboo(
  b: Bamboo, a: ArenaPixels, cx: number, cy: number, cr: number,
): boolean {
 ...
 */
export function randomSpot(existing: Bamboo[]): { nx: number; ny: number } | null {
  ...
  for (let attempt = 0; attempt < 24; attempt++) {
    ...
    if (existing.some((e) => Math.hypot(e.nx - nx, e.ny - ny) < MIN_SEP)) continue;
    return { nx, ny };
  }
  return null;
}
 ...
 */
export function drawBamboo(g: Phaser.GameObjects.Graphics, b: Bamboo, a: ArenaPixels): void {
  ...
  g.fillEllipse(x, y + caneH * 0.42, caneW * (1 + b.stage) * 1.1, unit * 0.55);
