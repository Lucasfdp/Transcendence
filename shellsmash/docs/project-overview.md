...
| File | Purpose |
|------|---------|
| `srcs/docker-compose.yml` | Production service definitions |
| `srcs/docker-compose.override.yml` | Dev overrides — hot-reload for frontend (Vite HMR) and backend (`nest start --watch`) |
...
**Path:** `srcs/requirements/reverse_proxy/`
...
**Path:** `srcs/requirements/backend/`
...
**Path:** `srcs/requirements/database/`
...
**Path:** `srcs/requirements/frontend/`

A single-page application built with [Phaser 3.60](https://phaser.io/) and bundled by [Vite](https://vitejs.dev/). No UI framework — all rendering is done with Phaser's Canvas/WebGL pipeline.

**Entry point:** `src/main.ts` — creates the Phaser `Game` instance with a 1280×720 canvas, registers all scenes, and boots into `AuthCallbackScene`.
...
**AuthCallbackScene** (`src/hub/AuthCallbackScene.ts`)  
Runs first on every page load. Checks for a `?token=` query parameter (from 42 OAuth callback), stores it in `localStorage`, cleans the URL with `globalThis.history.replaceState`, then transitions to `HubScene`.

**HubScene** (`src/hub/HubScene.ts`)  
...
| File | Purpose |
|------|---------|
| `src/hub/api.ts` | Typed wrappers around `fetch` — all API calls go through `apiFetch()` which attaches the JWT Bearer token automatically. Exports `api.getMe()`, `api.getAllUsers()`, `api.getMiniGames()`, `api.devLogin()`. |
| `src/hub/theme.ts` | Central colour palette (warm Japanese-temple: deep charcoal background, gold accents, muted red). All Phaser graphics calls reference `THEME.*` constants so colours are changed in one place. |
...
**File:** `srcs/requirements/frontend/src/src/hub/HubScene.ts`
...
**File:** `srcs/requirements/frontend/src/src/hub/ProfilePanel.ts`
...
- Points to `srcs/requirements/backend/src/coverage/lcov.info` for test coverage reporting.

### Jest

**Config:** `srcs/requirements/backend/src/jest.config.ts`  
...
**Config:** `srcs/requirements/backend/src/.eslintrc.js`
