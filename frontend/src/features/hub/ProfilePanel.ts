...
const INVENTORY_SHELL_TYPES = Object.values(PowerType).filter(
	(p) => p !== PowerType.NONE,
);
...
export class ProfilePanel {
	...
	private build(user: any, shellInventory?: Record<string, number>) {
		const PH =
			(shellInventory ? PH_BASE + INV_SECTION_H : PH_BASE) + PH_FRIENDS;
		...
		// Outer glow
		framGfx.fillStyle(THEME.gold, 0.07);
		framGfx.fillCircle(avatarCX, avatarCY, frameR + 14);
		framGfx.fillStyle(THEME.gold, 0.12);
		framGfx.fillCircle(avatarCX, avatarCY, frameR + 7);
		// Frame ring
		framGfx.lineStyle(4, THEME.gold, 1);
		framGfx.strokeCircle(avatarCX, avatarCY, frameR);
		// Dark inner fill (avatar background)
		framGfx.fillStyle(0x0d0a06, 1);
		framGfx.fillCircle(avatarCX, avatarCY, frameR - 2);
		children.push(framGfx);

		// ── 3. Placeholder turtle art ─────────────────────────────────────────────
		children.push(
			...this.drawTurtlePlaceholder(avatarCX, avatarCY, frameR - 6),
		);
		...
		badgeGfx.fillStyle(0x1a0d3a, 1);
		badgeGfx.fillCircle(badgeX, badgeY, 18);
		badgeGfx.lineStyle(2, THEME.gold, 0.9);
		badgeGfx.strokeCircle(badgeX, badgeY, 18);
		const badgeLabel = this.scene.add
			.text(badgeX, badgeY, "42", {
				fontSize: "11px",
		...
		beltGfx.lineStyle(1, rank.colour, 0.7);
		beltGfx.strokeRoundedRect(
			PW / 2 - beltW / 2,
			beltY,
			beltW,
			beltH,
		...
		const beltText = this.scene.add
			.text(PW / 2, beltY + beltH / 2, rank.label, {
				fontSize: "10px",
		...
		lvlGfx.fillStyle(THEME.gold, 1);
		lvlGfx.fillCircle(nameRight + 16, nameMidY, 16);
		lvlGfx.fillStyle(THEME.background, 1);
		lvlGfx.fillCircle(nameRight + 16, nameMidY, 13);
		lvlGfx.lineStyle(1.5, THEME.gold, 0.7);
		lvlGfx.strokeCircle(nameRight + 16, nameMidY, 16);
		const lvlText = this.scene.add
			.text(nameRight + 16, nameMidY, `${user.level ?? 1}`, {
				fontSize: "11px",
				color: THEME.textGold,
				fontFamily: THEME.font,
				fontStyle: "bold",
		...
		for (let v = 0; v < 6; v++) {
			...
			hexPoints.push({
				x: hexCX + Math.cos(a) * hexR,
				y: hexCY + Math.sin(a) * hexR,
			});
		}
		...
		children.push(
			this.scene.add.text(PAD, xpBarY - 14, "XP", {
				fontSize: "10px",
				color: THEME.textMutedHex,
				fontFamily: THEME.font,
				fontStyle: "bold",
		...
		stats.forEach((stat, i) => {
			...
			sg.lineStyle(1, THEME.gold, 0.3);
			sg.strokeRoundedRect(sx, statsY, slotW, 64, 6);
			children.push(
				sg,
				this.scene.add
					.text(sx + slotW / 2, statsY + 14, `${stat.value}`, {
						fontSize: "20px",
						color: THEME.textGold,
						fontFamily: THEME.font,
						fontStyle: "bold",
					...
					.setOrigin(0.5, 0),
				this.scene.add
					.text(sx + slotW / 2, statsY + 42, stat.label, {
						fontSize: "9px",
						color: THEME.textMutedHex,
						fontFamily: THEME.font,
						fontStyle: "bold",
		...
		if (shellInventory) {
			...
			// Section title
			children.push(
				this.scene.add.text(PAD, invStartY + 8, "SHELL INVENTORY", {
					fontSize: "10px",
					color: THEME.textMutedHex,
					fontFamily: THEME.font,
					fontStyle: "bold",
			...
			const chipW = Math.floor(
				(PW - PAD * 2 - INV_GAP * (INV_COLS - 1)) / INV_COLS,
			);
			...
			INVENTORY_SHELL_TYPES.forEach((shellType, idx) => {
				...
				invGfx.lineStyle(1, colour, isEmpty ? 0.2 : 0.55);
				...
				// Shell name
				children.push(
					this.scene.add.text(cx + 14, cy + 5, label, {
						fontSize: "8px",
				...
				children.push(
					this.scene.add
						.text(cx + chipW - 3, cy + 5, qtyLabel, {
							fontSize: "8px",
							color: isEmpty
								? THEME.textMutedHex
								: Phaser.Display.Color.IntegerToColor(colour)
										.rgba,
							fontFamily: THEME.font,
							fontStyle: "bold",
							align: "right",
		...
		this.friendsStatusText = this.scene.add
			.text(PW / 2, this.friendsBaseY + 6, "", {
				fontSize: "9px",
				color: THEME.textGold,
				fontFamily: THEME.font,
				align: "center",
		...
		this.addFriendDisplay = this.scene.add
			.text(inputX + 8, inputY + FR_INPUT_H / 2, "add by username…", {
				fontSize: "10px",
				color: THEME.textMutedHex,
				fontFamily: THEME.font,
			})
		...
		const paintClose = (hovered: boolean) => {
			...
			closeGfx.lineStyle(1, THEME.gold, hovered ? 0 : 0.4);
		...
		const closeTxt = this.scene.add
			.text(PW / 2, closeBtnY + 13, "✕  Close Profile", {
				fontSize: "13px",
				color: THEME.text,
				fontFamily: THEME.font,
				fontStyle: "bold",
		...
		closeHit.on("pointerover", () => {
			paintClose(true);
			closeTxt.setColor("#1a1410");
		});
		closeHit.on("pointerout", () => {
			paintClose(false);
			closeTxt.setColor(THEME.text);
		});
	...
	private async refreshFriends(): Promise<void> {
		try {
			...
			const errTxt = this.scene.add
				.text(
					PW / 2,
					this.friendsBaseY + 20,
	...
	private updateFriends(friends: FriendView[], pending: PendingView[]): void {
		...
		if (friends.length === 0) {
			...
			visible.forEach((friend) => {
				...
				const statusTxt = this.scene.add
					.text(PW - PAD - 32, curY + 2, statusStr, {
						fontSize: "9px",
				...
				const removeTxt = this.scene.add
					.text(PW - PAD - 2, curY + 2, "✕", {
						fontSize: "11px",
						color: THEME.textMutedHex,
						fontFamily: THEME.font,
					})
					.setOrigin(1, 0)
					.setInteractive({ useHandCursor: true });
				removeTxt.on("pointerover", () =>
					removeTxt.setColor("#cc4444"),
				);
				removeTxt.on("pointerout", () =>
					removeTxt.setColor(THEME.textMutedHex),
				);
				removeTxt.on("pointerup", () => {
					void api
						.removeFriend(friend.userId)
						.then(() => void this.refreshFriends());
				});
				objs.push(removeTxt);
			...
			if (friends.length > FR_MAX_VIS) {
				const moreTxt = this.scene.add.text(
					PAD + 10,
					curY + 2,
					`+ ${friends.length - FR_MAX_VIS} more`,
					{
						fontSize: "9px",
						color: THEME.textMutedHex,
						fontFamily: THEME.font,
		...
		if (pending.length > 0) {
			...
			visiblePending.forEach((req) => {
				...
				// Name
				const nameStr = (req.turtleName ?? req.username).substring(
					0,
					13,
				);
				const nameTxt = this.scene.add.text(
				...
				// Accept button
				const acceptTxt = this.scene.add
					.text(PW - PAD - 22, curY + 2, "✓", {
						fontSize: "12px",
						color: "#44cc44",
						fontFamily: THEME.font,
						fontStyle: "bold",
				...
				// Decline button
				const declineTxt = this.scene.add
					.text(PW - PAD - 2, curY + 2, "✕", {
						fontSize: "11px",
						color: THEME.textMutedHex,
						fontFamily: THEME.font,
					...
					.setOrigin(1, 0)
					.setInteractive({ useHandCursor: true });
				declineTxt.on("pointerover", () =>
					declineTxt.setColor("#cc4444"),
				);
				declineTxt.on("pointerout", () =>
					declineTxt.setColor(THEME.textMutedHex),
				);
				declineTxt.on("pointerup", () => {
					void api
						.removeFriend(req.userId)
						.then(() => void this.refreshFriends());
				});
			...
			if (pending.length > FR_PEND_MAX) {
				const morePendTxt = this.scene.add.text(
					PAD + 10,
					curY + 2,
					`+ ${pending.length - FR_PEND_MAX} more pending`,
					{
						fontSize: "9px",
						color: THEME.textMutedHex,
						fontFamily: THEME.font,
	...
	private showFriendsLoading(): void {
		this.clearFriendsLayer();
		const loadTxt = this.scene.add
			.text(PW / 2, this.friendsBaseY + 20, "Loading friends…", {
				fontSize: "10px",
				color: THEME.textMutedHex,
				fontFamily: THEME.font,
				align: "center",
	...
	private startAddFriendCapture(): void {
		...
		this.renderAddFriendBorder(
			this.addFriendInputX,
			this.addFriendInputY,
			this.addFriendInputW,
			FR_INPUT_H,
			true,
		);
	...
	private updateAddFriendDisplay(): void {
		...
		const cursor =
			this.addFriendCapturing && this.addFriendCursorVis ? "|" : "";
		const isEmpty = this.addFriendValue === "";
		const text = isEmpty
			? this.addFriendCapturing
				? cursor
				: "add by username…"
			: this.addFriendValue + cursor;
		this.addFriendDisplay
			.setText(text)
			.setColor(
				isEmpty && !this.addFriendCapturing
					? THEME.textMutedHex
					: THEME.text,
			);
	}
	...
	private renderAddFriendBorder(
		...
		this.addFriendBorderGfx.fillStyle(0x2a2218, 0.8);
		this.addFriendBorderGfx.fillRoundedRect(x, y, w, h, 4);
		this.addFriendBorderGfx.lineStyle(
			1,
			active ? THEME.gold : 0x4a3d2a,
			active ? 0.8 : 0.4,
		);
	...
	private drawTurtlePlaceholder(
		...
		g.fillCircle(cx - 6, eyeY, 4);
		g.fillStyle(0xffffff, 0.9);
		g.fillCircle(cx + 6, eyeY, 4);
		g.fillStyle(0x111111, 1);
		g.fillCircle(cx - 5, eyeY, 2);
		g.fillStyle(0x111111, 1);
		g.fillCircle(cx + 7, eyeY, 2);
		// Eye shine
		g.fillStyle(0xffffff, 0.8);
		g.fillCircle(cx - 4, eyeY - 1, 1);
		g.fillStyle(0xffffff, 0.8);
		g.fillCircle(cx + 8, eyeY - 1, 1);

		// ── Flippers (left + right) ──────────────────────────────────────────────
		g.fillStyle(0x3d6a1e, 0.85);
		g.fillEllipse(cx - shellW * 0.6, cy + 4, radius * 0.35, radius * 0.18);
		g.fillEllipse(cx + shellW * 0.6, cy + 4, radius * 0.35, radius * 0.18);
