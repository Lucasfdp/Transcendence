 ...
 *   public/assets/hub-background.png
...
export class HubScene extends Phaser.Scene {
	...
	private activeEmitter: Phaser.GameObjects.Particles.ParticleEmitter | null =
		null;
	...
	private modalKind: "default" | "achievements" | "customization" | null =
		null;
	private achievementWheelHandler:
		| ((
	...
	async create(): Promise<void> {
		...
		const isStale = () =>
			this._shutdownFired || this._createRunId !== runId;
		...
		try {
			this.minigames = await api.getMiniGames();
		} catch {
			this.minigames = [];
		}
	...
	private applyResize(): void {
		...
		if (this.modal && this.modalTitle !== null) {
			...
			if (kind === "achievements") void this.showAchievementsModal();
				...
				void this.showCustomizationModal();
	...
	private syncBackgroundImageOverlay(): void {
		...
		if (preset === "sunset_dojo") {
			...
			const { width, height } = this.scale;
			const scale = Math.max(width / SUNSET_BG_W, height / SUNSET_BG_H);
			if (this.bgImage?.active) {
				this.bgImage
					.setTexture("hub-sunset-bg")
	...
	private drawDefaultDojoBackground(): void {
		...
		lanternPositions.forEach((xFrac) => {
			...
			const ly = height * 0.3; // body centre
			const lw = Math.min(width, height) * 0.022; // half-width of body
			const lh = Math.min(width, height) * 0.04; // half-height of body
	...
	private drawSunsetDojoBackground(): void {
		...
		gfx.fillStyle(0xffb15f, 0.07);
		gfx.fillCircle(sunX, sunY, sunR * 3.1);
		gfx.fillStyle(0xffc16f, 0.13);
		gfx.fillCircle(sunX, sunY, sunR * 2.0);
		gfx.fillStyle(0xffd18a, 0.96);
		...
		const pathBotY = height;
		const cx = width / 2;
		pathGfx.fillStyle(0x382111, 0.9);
		pathGfx.fillPoints(
			[
				{ x: cx - pathTopW / 2, y: pathTopY },
				{ x: cx + pathTopW / 2, y: pathTopY },
		...
		[0.18, 0.38, 0.62, 0.82].forEach((xFrac) => {
			...
			lanternGfx.lineStyle(1, 0x54321d, 0.7);
			lanternGfx.lineBetween(lx, 0, lx, ly - lh);
		...
		for (let i = 0; i < 28; i++) {
			...
			const pw = Phaser.Math.Between(5, 11);
			const ph = Phaser.Math.Between(3, 7);
			const pc = Phaser.Math.RND.pick([
				0xffc0cb, 0xffd08a, 0xffa66a, 0xffe1b8, 0xffffff,
			]);
			petalGfx.fillStyle(pc, Phaser.Math.FloatBetween(0.18, 0.5));
			petalGfx.fillEllipse(px, py, pw, ph);
		}
	}

	/** Draw a simple cherry blossom tree silhouette. Pushes graphics into bgLayer. */
	private drawBlossamTree(
		x: number,
		baseY: number,
		height: number,
		leansRight: boolean,
	...
	private drawZoneIcon(
		...
		switch (id) {
			...
			case "river-rush": {
				...
				gfx.lineStyle(2 * u, 0x7ec8e3, 0.9);
				gfx.beginPath();
			...
			case "bamboo-bash": {
				...
				gfx.lineStyle(2 * u, 0x6ab04c, 0.9);
				[-4, 0, 4].forEach((dx) => {
					const bx = ix + 4 * u + dx * u;
					gfx.lineBetween(bx, iy, bx, iy + 14 * u);
					gfx.fillStyle(0x4a8a2c, 0.7);
	...
	private buildHotspots(): void {
		...
		HOTSPOTS.forEach((hs) => {
			...
			const available =
				minigame?.status === "available" || hs.id === "temple-curling";
			...
			// Gold outer border
			frameBg.lineStyle(2, THEME.gold, available ? 0.8 : 0.3);
			...
			const labelText = this.add
				.text(r.cx, r.cy, rawName.split(" ").join("\n"), {
			...
			if (!available) {
				...
				lockGfx.fillStyle(0x330000, 0.6);
			...
			zone.on("pointerover", () => {
				...
				this.glowGfx.lineStyle(2, 0xffffff, 0.9);
			...
			zone.on("pointerup", () => {
				...
				if (
					available &&
					this.scene.manager.getScene("ShellSmashArenaScene")
				) {
	...
	private drawExtrasSection(): void {
		...
		const title = this.add
			.text(panelX + panelW / 2, panelY + 13, "DOJO EXTRAS", {
	...
	private drawExtrasButton(
		...
		const paint = (hovered: boolean): void => {
			...
			btnGfx.lineStyle(1.5, THEME.gold, hovered ? 0.9 : 0.46);
			btnGfx.strokeRoundedRect(x, y, w, h, 7);
			btnGfx.fillStyle(THEME.gold, hovered ? 0.18 : 0.1);
	...
	private startPetals(
		...
		this.activeEmitter = this.add
			.particles(cx, cy - zoneH * 0.3, "petal", {
	...
	private showModal(title: string, description: string): void {
		...
		const icon = this.add
			.text(px, py - panelH / 2 + 38, "⛩", {
				fontSize: this.scaledFont(28),
			})
			.setOrigin(0.5);

		const nameText = this.add
			.text(px, py - panelH / 2 + 80, title.toUpperCase(), {
				fontSize: this.scaledFont(19),
				color: THEME.textGold,
				fontFamily: THEME.font,
				fontStyle: "bold",
				align: "center",
		...
		const soonText = this.add
			.text(px, py - panelH / 2 + 128, "Coming Soon", {
		...
		if (description) {
			const descText = this.add
				.text(px, py - panelH / 2 + 162, description, {
		...
		const closeBtn = this.add
			.text(px + panelW / 2 - 18, py - panelH / 2 + 16, "✕", {
				fontSize: this.scaledFont(15),
				color: THEME.textMutedHex,
				fontFamily: THEME.font,
			})
	...
	private async showAchievementsModal(): Promise<void> {
		...
		try {
			...
			if (this.modalKind === "achievements")
				this.renderAchievementsModal(achievements, null);
		} catch {
			if (this.modalKind === "achievements") {
	...
	private renderAchievementsModal(
		...
		const panelBlocker = this.add
			.rectangle(px, py, panelW, panelH, 0x000000, 0)
			.setInteractive();
		panelBlocker.on(
			"pointerdown",
			(
		...
		panelGfx.lineStyle(1, THEME.gold, 0.2);
		panelGfx.strokeRoundedRect(
			px - panelW / 2 + 6,
			py - panelH / 2 + 6,
			panelW - 12,
		...
		const title = this.add
			.text(px, py - panelH / 2 + 26, "ACHIEVEMENTS", {
				fontSize: this.scaledFont(22),
				color: THEME.textGold,
				fontFamily: THEME.font,
				fontStyle: "bold",
		...
		const subtitle = this.add
			.text(
				px,
				py - panelH / 2 + 58,
				"Dojo milestones unlocked by playing Shell Smash.",
				{
					fontSize: this.scaledFont(12),
					color: THEME.textMutedHex,
					fontFamily: THEME.font,
		...
		if (error) {
			...
			children.push(
				this.add
					.text(px, top + 80, "Loading achievements...", {
						fontSize: this.scaledFont(14),
						color: THEME.textMutedHex,
						fontFamily: THEME.font,
					})
			...
			summaryBar.fillStyle(0x050403, 0.7);
			summaryBar.fillRoundedRect(summaryX, summaryY, summaryW, 7, 4);
			summaryBar.fillStyle(THEME.gold, 0.9);
			summaryBar.fillRoundedRect(
				summaryX,
				summaryY,
				summaryW * ratio,
				7,
			...
			const contentHeight =
				rows * cardH + Math.max(0, rows - 1) * cardGap;
			...
			achievements.forEach((achievement, index) => {
				...
				listContainer.add(
					this.drawAchievementCard(x, y, cardW, cardH, achievement),
				);
			});
		...
		const closeBtn = this.add
			.text(px + panelW / 2 - 20, py - panelH / 2 + 18, "X", {
				fontSize: this.scaledFont(15),
				color: THEME.textMutedHex,
				fontFamily: THEME.font,
			})
	...
	private drawAchievementCard(
		...
		card.lineStyle(
			1.5,
			unlocked ? THEME.gold : 0x6d5940,
			unlocked ? 0.84 : 0.38,
		);
		card.strokeRoundedRect(x, y, w, h, 9);

		const badge = this.add
			.text(x + 18, y + 18, unlocked ? "MEDAL" : "LOCK", {
		...
		const desc = this.add.text(
			x + 18,
			y + 40,
			unlocked ? achievement.unlockDescription : achievement.description,
			{
		...
		const progressText = this.add
			.text(x + w - 18, y + h - 27, progress.label, {
				fontSize: this.scaledFont(9),
				color: unlocked ? THEME.textGold : THEME.textMutedHex,
				fontFamily: THEME.font,
				fontStyle: "bold",
		...
		const rewardText = rewardLabel
			? this.add.text(x + 18, y + h - 40, `Reward: ${rewardLabel}`, {
					fontSize: this.scaledFont(9),
					color: THEME.textGold,
					fontFamily: THEME.font,
					fontStyle: "bold",
	...
	private getAchievementProgress(achievement: Achievement): {
		ratio: number;
		label: string;
	} {
	...
	private async showCustomizationModal(): Promise<void> {
		...
		try {
			...
			if (this.modalKind === "customization")
				this.renderCustomizationModal(cosmetics, null);
		} catch {
			if (this.modalKind === "customization") {
				this.renderCustomizationModal(
					null,
					"Could not load customization. Try again later.",
				);
			}
		}
	}

	private renderCustomizationModal(
		cosmetics: Cosmetic[] | null,
		error: string | null,
	): void {
		const { width, height } = this.scale;
		const panelW = Math.min(740, width * 0.9);
		...
		const title = this.add
			.text(px, py - panelH / 2 + 26, "CUSTOMIZATION", {
				fontSize: this.scaledFont(22),
				color: THEME.textGold,
				fontFamily: THEME.font,
				fontStyle: "bold",
		...
		const subtitle = this.add
			.text(
				px,
				py - panelH / 2 + 58,
				"Unlock and equip shell skins and Hub backgrounds.",
				{
					fontSize: this.scaledFont(12),
					color: THEME.textMutedHex,
					fontFamily: THEME.font,
		...
		if (error) {
			...
			children.push(
				this.add
					.text(px, top + 80, "Loading cosmetics...", {
						fontSize: this.scaledFont(14),
						color: THEME.textMutedHex,
						fontFamily: THEME.font,
					})
			...
			const sections: { title: string; items: Cosmetic[] }[] = [
				{
					title: "Shell Skins",
					items: cosmetics.filter(
						(cosmetic) => cosmetic.type === "shell_skin",
					),
				},
				{
					title: "Hub Backgrounds",
					items: cosmetics.filter(
						(cosmetic) => cosmetic.type === "hub_background",
					),
				},
			];
			...
			sections.forEach((section) => {
				...
				section.items.forEach((cosmetic, index) => {
					...
					children.push(
						...this.drawCosmeticCard(x, y, cardW, cardH, cosmetic),
					);
				});
				yCursor +=
					Math.ceil(section.items.length / cols) * (cardH + cardGap) +
					10;
			});
		}
	...
	private drawCosmeticCard(
		...
		const enabled =
			cosmetic.owned || cosmetic.lockedReason === "purchasable";
		const card = this.add.graphics();
		card.fillStyle(
			cosmetic.equipped ? 0x1a1005 : 0x0b0806,
			cosmetic.owned ? 0.94 : 0.76,
		);
		card.fillRoundedRect(x, y, w, h, 9);
		card.lineStyle(
			1.5,
			cosmetic.equipped ? THEME.gold : 0x6d5940,
			cosmetic.equipped ? 0.94 : 0.4,
		);
		card.strokeRoundedRect(x, y, w, h, 9);
		...
		if (cosmetic.type === "hub_background") {
			preview.fillGradientStyle(
				0x2b174f,
				0x2b174f,
				accent,
				accent,
				cosmetic.owned ? 0.95 : 0.42,
			);
			preview.fillRoundedRect(x + 14, y + 18, 44, 36, 5);
			preview.fillStyle(
				cosmetic.previewColor ?? 0xfff5d6,
				cosmetic.owned ? 0.9 : 0.36,
			);
			preview.fillCircle(x + 46, y + 28, 7);
			preview.fillStyle(0x1a1005, cosmetic.owned ? 0.65 : 0.3);
			preview.fillTriangle(
				x + 14,
				y + 54,
				x + 28,
				y + 34,
		...
		if (enabled && !cosmetic.equipped) {
			...
			zone.on(
				"pointerup",
				() => void this.handleCosmeticAction(cosmetic),
			);
			objects.push(zone);
		}
		return objects;
	...
	private async handleCosmeticAction(cosmetic: Cosmetic): Promise<void> {
		try {
			...
			const equipped = cosmetics.find(
				(item) => item.type === cosmetic.type && item.equipped,
			);
			if (this.user && equipped?.type === "shell_skin")
				this.user.shellSkin = equipped.id;
			...
			if (this.modalKind === "customization")
				this.renderCustomizationModal(
					null,
					"Could not update customization.",
				);
		}
	}
	...
	private drawHUD(): void {
		...
		bar.fillStyle(THEME.background, 0.8);
		...
		bar.fillCircle(AVT_CX, AVT_CY, RING_R);
		...
		const levelLabel = this.add
			.text(
				PAD + 48,
				31,
				`Lvl ${this.user.level}  ·  Shell: ${this.user.shellSkin ?? "kanagawa"}`,
				{
					fontSize: this.scaledFont(11),
					color: THEME.text,
					fontFamily: THEME.font,
		...
		const coinsLabel = this.add
			.text(PAD + 48, 44, `⬡ ${this.user.coins ?? 0}`, {
				fontSize: this.scaledFont(10),
				color: THEME.textGold,
				fontFamily: THEME.font,
				fontStyle: "bold",
		...
		if (this.user.isDevAccount) {
			const devBadge = this.add
				.text(nameLabel.getRightCenter().x + 8, 10, "DEV", {
					fontSize: this.scaledFont(9),
					color: "#ffffff",
					fontFamily: THEME.font,
					fontStyle: "bold",
		...
		this.tweens.add({
			...
			ease: "Sine.easeOut",
			onComplete: () => {
				if (avatarRingGfx?.active) avatarRingGfx.destroy();
			},
		});
	...
	private drawLogoutButton(
		...
		sepGfx.lineStyle(1, THEME.gold, 0.2);
		sepGfx.lineBetween(
			btnX - txt.width - 12,
			10,
			btnX - txt.width - 12,
	...
	private async doLogout(): Promise<void> {
		try {
			...
			console.warn(
				"[HubScene] Logout request failed (continuing anyway):",
				err,
			);
		}
		this.scene.start("LandingScene");
	...
	private drawGuestBanner(): void {
		...
		const loginLink = this.add
			.text(
				msg.getRightCenter().x + 12,
				bannerY + BANNER_H / 2,
				"Log in to save",
				{
					fontSize: this.scaledFont(12),
		...
		loginLink.on("pointerout", () =>
			loginLink.setStyle({ textDecoration: "" }),
		);
		loginLink.on("pointerup", () => {
	...
	private renderLeaderboard(): void {
		...
		api.getLeaderboard(this.lbPeriod, scope)
			.then((entries: LeaderboardEntry[]) => {
				...
				tabs.forEach((tab, ti) => {
					...
					tabGfx.fillStyle(
						active ? THEME.gold : 0x2a2218,
						active ? 0.85 : 0.6,
					);
					tabGfx.fillRoundedRect(tx, tabY, tabW - 1, TAB_H - 2, 4);
					const tabTxt = this.add
						.text(
							tx + tabW / 2,
							tabY + (TAB_H - 2) / 2,
							tab.label,
				...
				togGfx.fillStyle(friendsOn ? THEME.green : 0x2a2218, 0.8);
				togGfx.fillRoundedRect(toggleX, toggleY, toggleW, TOGGLE_H, 3);
				togGfx.lineStyle(1, friendsOn ? THEME.green : THEME.gold, 0.4);
				togGfx.strokeRoundedRect(
					toggleX,
					toggleY,
					toggleW,
					TOGGLE_H,
				...
				const header = this.add
					.text(panelX - panelW / 2, headerY, "DOJO RANKINGS", {
						fontSize: this.scaledFont(10),
						color: THEME.textGold,
						fontFamily: THEME.font,
						fontStyle: "bold",
				...
				displayRows.forEach((entry, i) => {
					...
					const winsLabel = this.add
						.text(panelX - 12, rowY, `${entry.wins}W`, {
							fontSize: this.scaledFont(11),
							color: colour,
							fontFamily: THEME.font,
						})
			...
			.catch(() => {
				...
				errBg.fillStyle(THEME.background, 0.8);
				errBg.fillRoundedRect(
					panelX - panelW,
					panelY,
					panelW,
					panelH,
