...
export function drawBackground(
  ...
  [0.18, 0.38, 0.62, 0.82].forEach((xFrac) => {
    ...
    const light = scene.add.pointlight(lx, ly + lh * 0.5, 0xff6600, 60, 0.40, 0.06);
    track(light);
  });

  // ── Cherry blossom trees ──────────────────────────────────────────────────
  drawBlossomTree(scene, width * 0.06, height * 0.72, height * 0.38, true,  depth, created);
  drawBlossomTree(scene, width * 0.94, height * 0.72, height * 0.38, false, depth, created);
  ...
  for (let i = 0; i < 28; i++) {
    ...
    petalGfx.fillStyle(Phaser.Math.RND.pick(PETAL_COLOURS), Phaser.Math.FloatBetween(0.2, 0.55));
