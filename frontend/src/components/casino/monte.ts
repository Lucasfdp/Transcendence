...
/** Deterministic adjacent/non-adjacent swap pairs for the three-cup shuffle. */
export function monteSwapPairs(total: number): [number, number][] {
	...
	return Array.from({ length: total }, (_, index) => pairs[index % pairs.length]);
}
