...
describe("computeCardTilt", () => {
	it("should return zero rotation and centered shine when the pointer is at the card's center", () => {
	...
	it("should tilt right (positive rotateY) when the pointer is past the horizontal center", () => {
		const tilt = computeCardTilt(1, 0.5);
		expect(tilt.rotateY).toBeGreaterThan(0);
	});

	it("should tilt left (negative rotateY) when the pointer is before the horizontal center", () => {
		const tilt = computeCardTilt(0, 0.5);
		expect(tilt.rotateY).toBeLessThan(0);
	});

	it("should tilt back (positive rotateX) when the pointer is above the vertical center", () => {
	...
	it("should clamp out-of-range normalized coordinates into 0..1 before computing shine position", () => {
		const tilt = computeCardTilt(1.5, -0.5);
		expect(tilt.shineX).toBe(100);
		expect(tilt.shineY).toBe(0);
	});

	it("should default to a centered tilt when given NaN input instead of throwing", () => {
