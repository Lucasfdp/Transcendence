...
export class RedisHealthIndicator extends HealthIndicator {
  ...
  private tcpPing(host: string, port: number, password: string): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      socket.on('timeout', () => done(new Error(`TCP timeout after ${REDIS_TIMEOUT_MS} ms`)));
        ...
        if (password) {
          ...
          socket.once('data', (data) => {
            ...
            if (reply.startsWith('+OK')) {
              ...
              done(new Error(`Redis AUTH failed: ${reply.trim()}`));
            }
          });
        } else {
          ...
          socket.once('data', (data) => {
            ...
            if (reply.includes('PONG')) {
              ...
              done(new Error(`Redis PING failed: ${reply.trim()}`));
            }
          });
        }
      });
