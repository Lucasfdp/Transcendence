...
describe("AchievementsService", () => {
	...
	beforeEach(async () => {
		const mockRepo = {
			...
			create: jest.fn(
				(data: Partial<UserAchievement>) => data as UserAchievement,
			),
	...
	it("unlocks first match milestone only once", async () => {
		...
		expect(
			firstPass.some(
				(achievement) => achievement.id === "matches-1-played",
			),
		).toBe(true);

		repo.find.mockResolvedValueOnce(
			firstPass.map((achievement) => makeRecord(user, achievement.id)),
		);
	...
	it("unlocks ten-match milestone when gamesPlayed reaches ten", async () => {
		...
		expect(
			unlocked.some(
				(achievement) => achievement.id === "matches-10-played",
			),
		).toBe(true);
		expect(cosmeticsRepo.save).toHaveBeenCalledWith(
			expect.objectContaining({ cosmeticId: "bamboo" }),
		);
	});
	...
	it("unlocks dragon shell at fifty matches", async () => {
		...
		expect(
			unlocked.some(
				(achievement) => achievement.id === "matches-50-played",
			),
		).toBe(true);
		expect(cosmeticsRepo.save).toHaveBeenCalledWith(
	...
	it("unlocks per-game achievements from game stats context", async () => {
		const user = makeUser({ profile: makeProfile({ gamesPlayed: 1 }) });
		gameStatsRepo.find.mockResolvedValueOnce([
			{
				user,
				gameId: "kame-knock",
				gamesPlayed: 1,
		...
		expect(
			unlocked.some(
				(achievement) => achievement.id === "kame-knock-1-played",
			),
		).toBe(true);
	});
		...
		for (const mode of expectedModes) {
			for (const target of expectedTargets) {
				expect(
					achievements.some(
						(achievement) =>
							achievement.id === `${mode}-${target}-played`,
					),
				).toBe(true);
			}
		}
	...
	it("lists general match, level, and dojo coin milestones", async () => {
		...
		for (const target of progressTargets) {
			expect(
				achievements.some(
					(achievement) =>
						achievement.id === `matches-${target}-played`,
				),
			).toBe(true);
		}
		...
		for (const target of levelTargets) {
			expect(
				achievements.some(
					(achievement) =>
						achievement.id === `level-${target}-reached`,
				),
			).toBe(true);
		}
		...
		for (const target of coinTargets) {
			expect(
				achievements.some(
					(achievement) =>
						achievement.id === `dojo-coins-${target}-earned`,
				),
			).toBe(true);
		}
	});
	...
	it("grants coin rewards once when an achievement unlocks", async () => {
		...
		expect(
			unlocked.some(
				(achievement) => achievement.id === "dojo-coins-1-earned",
			),
		).toBe(true);
	...
	it("includes backend progress values in achievement views", async () => {
		const user = makeUser({
			profile: makeProfile({ gamesPlayed: 4, totalWins: 0 }),
		});
		...
		expect(
			achievements.find(
				(achievement) => achievement.id === "matches-5-played",
			),
		).toEqual(
	...
	it("does not duplicate achievements or cosmetic rewards on duplicate evaluation", async () => {
		const user = makeUser({
			profile: makeProfile({ gamesPlayed: 1, totalWins: 1 }),
		});
	...
	it("ignores duplicate cosmetic reward rows safely", async () => {
		...
		const user = makeUser({
			profile: makeProfile({ gamesPlayed: 50, totalWins: 1 }),
		});
