...
export class AchievementsService {
	...
	async listForUser(user: User): Promise<AchievementView[]> {
		...
		return ACHIEVEMENTS.map((achievement) =>
			this.toView(achievement, context, unlocked.get(achievement.id)),
		);
	}
	...
	private async grantCosmetic(user: User, cosmeticId: string): Promise<void> {
		if (!findCosmetic(cosmeticId))
			throw new InternalServerErrorException("Invalid cosmetic reward");

		try {
			await this.userCosmeticsRepo.save(
				this.userCosmeticsRepo.create({ user, cosmeticId }),
			);
		} catch (err: unknown) {
			if ((err as { code?: string })?.code === "23505") return;
			throw new InternalServerErrorException(
				"Failed to unlock cosmetic reward",
			);
		}
	}
}
