import {
  BadRequestException, ForbiddenException, Injectable, InternalServerErrorException, NotFoundException,
} from '@nestjs/common';
...
export class CustomizationService {
  ...
  async buy(user: User, cosmeticId: string): Promise<CosmeticView[]> {
    const cosmetic = this.requireCosmetic(cosmeticId);
    if (cosmetic.defaultUnlocked) throw new BadRequestException('Default cosmetics cannot be bought');
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const currentUser = await usersRepo.findOne({ where: { id: user.id }, relations: ['profile'] });
      if (!currentUser) throw new ForbiddenException('User not found');

      const ownedIds = await this.findOwnedIds(currentUser.id, cosmeticsRepo);
      const achievementIds = await this.findAchievementIds(currentUser.id, achievementsRepo);
      if (this.isOwned(cosmetic, ownedIds)) return this.toViews(currentUser, ownedIds, achievementIds);

      if (cosmetic.unlockAchievementId) {
        if (!achievementIds.has(cosmetic.unlockAchievementId)) {
          throw new ForbiddenException('Cosmetic is locked by achievement');
        }
      }

      if (currentUser.coins < cosmetic.price) throw new BadRequestException('Not enough coins');
  ...
  private async findOwnedIds(
    ...
    const rows = await repo.find({ where: { user: { id: userId } }, relations: ['user'] });
  ...
  private isOwned(cosmetic: CosmeticDefinition, ownedIds: Set<string>): boolean {
    return cosmetic.defaultUnlocked === true || ownedIds.has(cosmetic.id);
  }

  private toViews(user: User, ownedIds: Set<string>, achievementIds: Set<string>): CosmeticView[] {
    return COSMETICS.map((cosmetic) => {
      const owned = this.isOwned(cosmetic, ownedIds);
      const lockedReason = this.lockedReason(user, cosmetic, owned, achievementIds);
      return {
        ...cosmetic,
        owned,
