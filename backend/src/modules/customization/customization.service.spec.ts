...
describe('CustomizationService', () => {
  ...
  beforeEach(async () => {
    ...
    dataSource = {
      transaction: jest.fn(async (callback: (manager: { getRepository: (entity: unknown) => unknown }) => unknown) => callback({
        getRepository: (entity: unknown) => {
          ...
          if (entity === UserAchievement) return achievementsRepo;
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(UserCosmetic), useValue: cosmeticsRepo },
        { provide: getRepositoryToken(UserAchievement), useValue: achievementsRepo },
  ...
  it('marks default kanagawa as owned', async () => {
    ...
    expect(cosmetics.find((cosmetic) => cosmetic.id === 'kanagawa')).toEqual(
      expect.objectContaining({ owned: true, equipped: true }),
    );
    expect(cosmetics.find((cosmetic) => cosmetic.id === 'default_dojo')).toEqual(
      expect.objectContaining({ owned: true, equipped: true }),
    );
  });
  ...
  it('cannot equip locked cosmetic', async () => {
    ...
    await expect(service.equip(user, 'dragon')).rejects.toThrow(ForbiddenException);
  });
  ...
  it('can equip owned cosmetic', async () => {
    ...
    expect(usersRepo.save).toHaveBeenCalledWith(expect.objectContaining({ shellSkin: 'dragon' }));
    expect(cosmetics.find((cosmetic) => cosmetic.id === 'dragon')?.equipped).toBe(true);
  });

  it('can equip owned hub background', async () => {
    const user = makeUser();
    cosmeticsRepo.find = jest.fn().mockResolvedValue([makeCosmetic(user, 'sunset_dojo')]);

    const cosmetics = await service.equip(user, 'sunset_dojo');

    expect(usersRepo.save).toHaveBeenCalledWith(expect.objectContaining({ hubBackground: 'sunset_dojo' }));
    expect(cosmetics.find((cosmetic) => cosmetic.id === 'sunset_dojo')?.equipped).toBe(true);
    expect(cosmetics.find((cosmetic) => cosmetic.id === 'kanagawa')?.equipped).toBe(true);
  });

  it('cannot buy unknown cosmetic', async () => {
    const user = makeUser();

    await expect(service.buy(user, 'missing')).rejects.toThrow(NotFoundException);
  });
  ...
  it('cannot buy achievement-locked cosmetic before achievement', async () => {
    ...
    await expect(service.buy(user, 'dragon')).rejects.toThrow(ForbiddenException);
  });
  ...
  it('can buy eligible cosmetic when enough coins', async () => {
    ...
    expect(cosmetics.find((cosmetic) => cosmetic.id === 'dragon')?.owned).toBe(true);
  });
