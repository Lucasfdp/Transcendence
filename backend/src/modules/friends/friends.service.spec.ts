...
const makeUser = (overrides: Partial<User> = {}): User =>
  Object.assign(new User(), {
    id: 1, username: 'kame', turtleName: 'KameMaster', shellSkin: 'kanagawa',
    avatar: null, level: 5, xp: 0, coins: 0, isGuest: false, ...overrides,
  });
...
describe('FriendsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        FriendsService,
        { provide: getRepositoryToken(Friendship), useFactory: mockFriendshipRepo },
  ...
  describe('sendRequest', () => {
    it('should throw NotFoundException when addressee username does not exist', async () => {
      ...
      await expect(service.sendRequest(1, 'ghost')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException when sending a request to yourself', async () => {
      ...
      await expect(service.sendRequest(1, 'self')).rejects.toThrow(BadRequestException);
    });

    it('should throw ConflictException when a friendship row already exists', async () => {
      userRepo.findOne.mockResolvedValue(makeUser({ id: 2, username: 'rival' }));
      friendshipRepo.findOne.mockResolvedValue({ id: 1 });
      await expect(service.sendRequest(1, 'rival')).rejects.toThrow(ConflictException);
    });
    ...
    it('should create a pending friendship on the happy path', async () => {
      ...
      expect(friendshipRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ requesterId: 1, addresseeId: 2, status: 'pending' }),
      );
    });
  });
  ...
  describe('listFriends', () => {
    it('should return FriendView with isOnline from PresenceService', async () => {
      ...
      friendshipRepo.find.mockResolvedValue([
        { requesterId: 1, addresseeId: 2, status: 'accepted', requester: makeUser({ id: 1 }), addressee: alice },
      ]);
