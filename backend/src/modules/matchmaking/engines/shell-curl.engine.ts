...
import { CurlingSnapshot, GameInputPayload, MatchRoom, RoomPlayer } from '../matchmaking.types';
...
export class ShellCurlEngine implements GameEngine {
  ...
  createInitialState(context: GameEngineCreateContext, roomPlayers: RoomPlayer[]): CurlingSnapshot {
  ...
  private applyRelease(room: MatchRoom, userId: number, payload: Record<string, unknown> = {}): MatchRoom | null {
    ...
    if (state.objects.some((object) => object.id === state.turnNumber)) return null;
  ...
  private applySettled(room: MatchRoom, userId: number, payload: Record<string, unknown> = {}): MatchRoom | null {
    ...
    if (state.throwsInEnd >= room.players.length * state.stonesPerPlayer) {
      ...
      if (endScore.scoringSide !== null) state.score[endScore.scoringSide] += endScore.points;
      state.currentEnd += 1;
      state.throwsInEnd = 0;
      state.objects = [];
    }
    ...
    if (state.currentEnd >= state.totalEnds || state.turnNumber >= state.maxTurns) {
  ...
  private finish(room: MatchRoom, winnerSide: number | null, abandoned = false): void {
