...
function makePlayer(side: number, shellSelection: string[] = []): RoomPlayer {
	return {
		...
		side, shellSelection, ready: true, connected: true,
	};
}

function makeRoom(playerCount = 2, selections: string[][] = []): MatchRoom {
	const engine = new ShellCurlEngine();
	const players = Array.from({ length: playerCount }, (_value, side) => makePlayer(side, selections[side] ?? []));
	const state = engine.createInitialState({
		matchId: "match-curl", gameId: "temple-curling", mode: "casual", powerupsEnabled: true,
...
function release(engine: ShellCurlEngine, room: MatchRoom, userId: number, payload: Record<string, unknown>): MatchRoom | null {
	return engine.handleInput(room, userId, { matchId: room.matchId, action: "release", payload });
}
...
function settle(engine: ShellCurlEngine, room: MatchRoom): void {
	for (let step = 0; step < 1_000; step++) {
		...
		if (!room.physicsState?.entities.some((entity) => !entity.stopped)) return;
	}
...
describe("ShellCurlEngine", () => {
	...
	it("owns source-space movement and rejects client settlement claims", () => {
		...
		const room = makeRoom();
		engine.start(room);
		expect(release(engine, room, 1, { x: 1, y: 1, vx: 300, vy: 0 })).toBe(room);
		expect(room.physicsState?.entities[0]).toMatchObject({ x: 90, y: 440 });
		expect(engine.handleInput(room, 1, { matchId: room.matchId, action: "settled", payload: { objects: [] } })).toBeNull();
		engine.advanceSimulation(room, 1000 / 30);
	...
	it.each([2, 5])("preserves three ends and turn order for %i players", (playerCount) => {
		const engine = new ShellCurlEngine();
