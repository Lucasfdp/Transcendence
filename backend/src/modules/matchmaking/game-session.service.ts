...
export class GameSessionService {
  constructor(
    ...
    @InjectRepository(UserRating) private readonly ratingRepo: Repository<UserRating>,
  ) {}
  ...
  async startIfReady(matchId: string): Promise<MatchRoom | null> {
    ...
    if (!room || room.status !== 'pending' || !room.players.every((player) => player.ready)) return room;
    const started = this.roomService.start(matchId);
    await this.matchRepo.update(matchId, { status: 'active', startedAt: new Date() });
    return started;
  }
  ...
  private async persistFinishedRoom(room: MatchRoom, abandoned: boolean): Promise<void> {
    const winnerSide = room.state.winnerSide;
    const winnerUserId = winnerSide === null ? null : room.players[winnerSide].user.id;
    ...
    for (const player of room.players) {
      const outcome = winnerSide === null ? 'draw' : player.side === winnerSide ? 'win' : abandoned ? 'abandoned' : 'loss';
      await this.matchPlayerRepo.update({ matchId: room.matchId, userId: player.user.id }, { outcome });
      ...
      if (user && !user.isGuest && outcome !== 'draw') {
        await this.gameResultsService.submitResult(user, {
          gameId: room.gameId === 'shell-curl' ? 'temple-curling' : room.gameId,
          outcome: outcome === 'win' ? 'win' : 'loss',
        });
      }
    }

    if (room.mode === 'ranked' && winnerSide !== null) await this.updateRatings(room, winnerSide);
  }

  private async updateRatings(room: MatchRoom, winnerSide: number): Promise<void> {
    for (const player of room.players) {
      let rating = await this.ratingRepo.findOne({ where: { userId: player.user.id, gameId: room.gameId } });
      if (!rating) rating = this.ratingRepo.create({ userId: player.user.id, gameId: room.gameId });
